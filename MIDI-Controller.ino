...
void loop() {
  for (int i = 0; i < 5; i++) {
    ...
    if (buttonStates[i] != lastButtonState[i]) {
      if (buttonStates[i] == HIGH && buttonPins[i] == 2) {
      ...
      else if (buttonStates[i] == HIGH && buttonPins[i] == 6) {
        noteOn(0x90, 0x3C, 0x7F);
        digitalWrite(ledPin, HIGH);
      }
      else {
        if (buttonStates[i] == LOW && buttonPins[i] == 2) {
        ...
        else if (buttonStates[i] == LOW && buttonPins[i] == 6) {
